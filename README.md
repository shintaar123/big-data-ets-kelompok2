...
| Salsa Bil Ulla (5027241052) | RSS Producer & Consumer HDFS | Membuat `kafka/producer_rss.py` untuk mengambil berita gempa dari RSS feed secara berkala dengan pencegahan duplikasi via persistent cache; serta `kafka/consumer_to_hdfs.py` untuk membaca dari kedua topic Kafka secara paralel (threading), mengirim notifikasi Telegram untuk gempa signifikan, dan menyimpan data ke HDFS maupun file lokal dashboard. |
